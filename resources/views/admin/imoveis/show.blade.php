...
@section('conteudo')
    ...
    @include('admin.imoveis.partials.modal_comissao')
    ...
    <div class="container p-0 p-md-5">
        ...
        <div class="row  justify-content-between bg-white py-2">
            <div class="col-auto">
                @if ($imovel->status == 'ativo')
                    <form action="{{ route('admin.imoveis.desativar') }}" method="post">
                        @csrf
                        <input type="hidden" name="id" value="{{ $imovel->id }}">
                        <button type="submit" class="btn btn-primary">Marcar como pendente</button>
                    </form>
                @else
                    <form action="{{ route('admin.imoveis.ativar') }}" method="post">
                        @csrf
                        <input type="hidden" name="id" value="{{ $imovel->id }}">
                        <button type="submit" class="btn btn-primary">Marcar como Ativo</button>
                    </form>
                @endif
            </div>
            <div class="col-auto">
                <div class="row g-2">
                    <div class="col-auto"><a href="{{ route('admin.imoveis.edit', $imovel->id) }}"
        ...
        <div class="row bg-white p-3 mt-4">
            <div class="col-12">
                ...
                <h5><span class="fw-bold">Usuário: </span> 
                    <a href="{{route('admin.usuarios.show', $imovel->user_id)}}">{{ $imovel->user_id }}</a>
                </h5>
            </div>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="row">
                            <div class="col">
                                ...
                                <p><span class="fw-bold">Preço do imóvel(AKZ): </span>
                                ...
                                @php
                                    ...
                                    $comissao_corretor = ($comissao_imobiliaria * $imovel->comissao_corretor) / 100;
                                @endphp
                                <p><span class="fw-bold">Comissao Imobiliária(AKZ): </span>
                                    {{ number_format($comissao_imobiliaria, 2, ',', '.') }} </p>
                                <p><span class="fw-bold">Comissão Corretor(%): </span> {{ $imovel->comissao_corretor }}
                                </p>
                                <p class="mb-1"><span class="fw-bold">Comissão Corretor(AKZ): </span>
                                    {{ number_format($comissao_corretor, 2, ',', '.') }} </p>
                                    <p class="text-danger m-0" style="font-size: 0.7em">A comissão do corretor depende da comissão da imobiliária</p>
                            </div>
                            <div class="col-auto">
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                    data-bs-target="#modal_comissao">
                                    Comissão
                                </button>
                            </div>
                        </div>
        ...
        <div class="row mt-4 my-md-5 bg-white">
